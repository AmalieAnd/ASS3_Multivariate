...
pacman::p_load("coda", "mvtnorm", "devtools", "rstan", "rethinking", "dplyr", "dfcrm", "brms", "ggplot2")
# library(devtools)
...
```{r all measures - interaction effect}
#Model
flist <-  alist(
    ADOS.s ~ dnorm(mu , sigma),
    mu <- a + bV*VerbalIQ.s + bN*NonVerbalIQ.s + bS*SocialIQ.s ,
...
m3.3 <- rethinking::map(flist, data = visit1)
precis(m3.2)
precis_plot( precis(m3.2))
